...
/*
  Note: In the following routines, "adjoint" means Mcomplex conjugate
  transpose:
  A* = conjugate(A^T)
  */
...
{
    u_int	i, /* j, */ k, m, n, p;
    Mcomplex	**A_v, **B_v /*, *B_row, *OUT_row, sum, tmp */;
...
{
    int	i, k, limit;
    /* Mcomplex	*B_row, *OUT_row, multiplier; */
    Mcomplex	tmp;
...
{
    u_int	i, m, n;
    Mcomplex	**A_v, *b_v /*, *A_row */;
    /* register Mcomplex	sum; */
...
Mcomplex	s;
{
    /* register Mcomplex	*A1_e, *A2_e, *out_e; */
    /* register int	j; */
    int	i, m, n;
...
ZVEC	*vec;
{
	...
	MEMCOPY(mat->me[row],vec->ve,lim,Mcomplex);
...
int	row;
{
	...
	MEMCOPY(vec->ve,mat->me[row],lim,Mcomplex);

	return (mat);
}

/* zm_rand -- randomise a Mcomplex  matrix; uniform in [0,1)+[0,1)*i */
ZMAT	*zm_rand(A)
ZMAT	*A;
{
    int		i;
