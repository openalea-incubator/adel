...
#ifndef SEGMENTED
   if ((matrix->base = NEW_A(m*n,Mcomplex)) == (Mcomplex *)NULL )
   {
      free(matrix);
      error(E_MEM,"zm_get");
   }
   else if (mem_info_is_on()) {
      mem_bytes(TYPE_ZMAT,0,m*n*sizeof(Mcomplex));
   }
#else
   matrix->base = (Mcomplex *)NULL;
#endif
   if ((matrix->me = (Mcomplex **)calloc(m,sizeof(Mcomplex *))) == 
       (Mcomplex **)NULL )
   {	free(matrix->base);	free(matrix);
	error(E_MEM,"zm_get");
     }
   else if (mem_info_is_on()) {
      mem_bytes(TYPE_ZMAT,0,m*sizeof(Mcomplex *));
   }
#ifndef SEGMENTED
   /* set up pointers */
   for ( i=0; i<m; i++ )
...
#else
   for ( i = 0; i < m; i++ )
     if ( (matrix->me[i]=NEW_A(n,Mcomplex)) == (Mcomplex *)NULL )
       error(E_MEM,"zm_get");
     else if (mem_info_is_on()) {
	mem_bytes(TYPE_ZMAT,0,n*sizeof(Mcomplex));
     }
#endif
...
int	size;
{
   ...
   if ((vector->ve=NEW_A(size,Mcomplex)) == (Mcomplex *)NULL )
   {
      free(vector);
      error(E_MEM,"zv_get");
   }
   else if (mem_info_is_on()) {
      mem_bytes(TYPE_ZVEC,0,size*sizeof(Mcomplex));
   }
   return (vector);
}
...
#ifndef SEGMENTED
   if ( mat->base != (Mcomplex *)NULL ) {
      if (mem_info_is_on()) {
	 mem_bytes(TYPE_ZMAT,mat->max_m*mat->max_n*sizeof(Mcomplex),0);
      }	   
      free((char *)(mat->base));
   }
#else
   for ( i = 0; i < mat->max_m; i++ )
     if ( mat->me[i] != (Mcomplex *)NULL ) {
	if (mem_info_is_on()) {
	   mem_bytes(TYPE_ZMAT,mat->max_n*sizeof(Mcomplex),0);
	}
	free((char *)(mat->me[i]));
     }
#endif
   if ( mat->me != (Mcomplex **)NULL ) {
      if (mem_info_is_on()) {
	 mem_bytes(TYPE_ZMAT,mat->max_m*sizeof(Mcomplex *),0);
      }	   
      free((char *)(mat->me));
   }
...
ZVEC	*vec;
{
   ...
   {
      if (mem_info_is_on()) {
	 mem_bytes(TYPE_ZVEC,vec->max_dim*sizeof(Mcomplex)+
		      sizeof(ZVEC),0);
	 mem_numvar(TYPE_ZVEC,-1);
      }
...
int	new_m, new_n;
{
   ...
   {	/* re-allocate A->me */
      if (mem_info_is_on()) {
	 mem_bytes(TYPE_ZMAT,A->max_m*sizeof(Mcomplex *),
		      new_m*sizeof(Mcomplex *));
      }

      A->me = RENEW(A->me,new_m,Mcomplex *);
      if ( ! A->me )
   ...
   {	/* re-allocate A->base */
      if (mem_info_is_on()) {
	 mem_bytes(TYPE_ZMAT,A->max_m*A->max_n*sizeof(Mcomplex),
		new_size*sizeof(Mcomplex));      
      }

      A->base = RENEW(A->base,new_size,Mcomplex);
      if ( ! A->base )
	error(E_MEM,"zm_resize");
      A->max_size = new_size;
   }
   ...
   if ( A->max_n < new_n )
   {
      ...
      {
	 if (mem_info_is_on()) {
	    mem_bytes(TYPE_ZMAT,A->max_n*sizeof(Mcomplex),
			 new_max_n*sizeof(Mcomplex));
	 }

	 if ( (tmp = RENEW(A->me[i],new_max_n,Mcomplex)) == NULL )
	   error(E_MEM,"zm_resize");
	 else {
	    A->me[i] = tmp;
	 }
      ...
      {
	 if ( (tmp = NEW_A(new_max_n,Mcomplex)) == NULL )
	   error(E_MEM,"zm_resize");
	 else {
	    A->me[i] = tmp;
	    if (mem_info_is_on()) {
	       mem_bytes(TYPE_ZMAT,0,new_max_n*sizeof(Mcomplex));
	    }
	 }
      }
   }
   else if ( A->max_m < new_m )
   {
	...
	if ( (A->me[i] = NEW_A(new_max_n,Mcomplex)) == NULL )
	  error(E_MEM,"zm_resize");
	else if (mem_info_is_on()) {
	   mem_bytes(TYPE_ZMAT,0,new_max_n*sizeof(Mcomplex));
	}
...
/* zv_resize -- returns the (Mcomplex) vector x with dim new_dim
...
int	new_dim;
{
   ...
   {
      if (mem_info_is_on()) { 
	 mem_bytes(TYPE_ZVEC,x->max_dim*sizeof(Mcomplex),
		      new_dim*sizeof(Mcomplex));
      }

      x->ve = RENEW(x->ve,new_dim,Mcomplex);
      if ( ! x->ve )
