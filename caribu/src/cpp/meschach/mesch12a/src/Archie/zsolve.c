...
double	diag;
{
    u_int	dim /* , j */;
    int	i, i_lim;
    Mcomplex	**mat_ent, *mat_row, *b_ent, *out_ent, *out_col, sum;
...
double	diag;
{
	...
	/* MEM_COPY(&(b_ve[i_lim]),&(out_ve[i_lim]),
	   (dim-i_lim)*sizeof(Mcomplex)); */
	MEMCOPY(&(b_ve[i_lim]),&(out_ve[i_lim]),dim-i_lim,Mcomplex);
    }
...
double	diag;
{
    ...
    {
	__zzero__(out_ve,out->dim);
	/* MEM_COPY(b_ve,out_ve,(i_lim+1)*sizeof(Mcomplex)); */
	MEMCOPY(b_ve,out_ve,i_lim+1,Mcomplex);
    }
