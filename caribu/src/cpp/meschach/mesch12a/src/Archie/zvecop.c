...
/* _zin_prod -- inner product of two vectors from i0 downwards
	...
	-- flag == 0 means compute sum_i a[i].b[i] */
Mcomplex	_zin_prod(a,b,i0,flag)
ZVEC	*a,*b;
u_int	i0, flag;
{
	u_int	limit;
...
{
	/* u_int	i, dim; */
	/* Mcomplex	*out_ve, *vec1_ve, *vec2_ve; */
...
/* zv_linlist -- linear combinations taken from a list of arguments;
   ...
   where vi are vectors (ZVEC *) and ai are numbers (Mcomplex)
*/

ZVEC	*zv_linlist(ZVEC *out,ZVEC *v1,Mcomplex a1,...)
{
   va_list ap;
...
/* zv_rand -- randomise a Mcomplex vector; uniform in [0,1)+[0,1)*i */
ZVEC	*zv_rand(x)
ZVEC	*x;
{
    if ( ! x )
