...
class clean_can(object):
    ...
    def __call__(self, name, surf_lim, hlim):
        ...
        for i in range(1,len(tab_geom)-1):
            if triangle_area(tab_geom[i][5:8], tab_geom[i][8:11], tab_geom[i][11:])<surf_lim and abs(tab_geom[i][7])<hlim and abs(tab_geom[i][10])<hlim and abs(tab_geom[i][13])<hlim: #test pour ecarter triangles bugge
