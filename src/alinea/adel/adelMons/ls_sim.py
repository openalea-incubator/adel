...
class ls_sim:
    ...
    def __call__(self, ls_file):
        ...
        for i in range(len(ls['TT'])):
            ls_par.append('.\GenPars'+'\\'+ls['annee'][i][-2:]+'_geom'+ls['gen'][i]+'_'+ls['semis'][i]+ls['bloc'][i]+'.h')
            ls_can.append('temp'+'\\'+ls['gen'][i]+'_'+ls['semis'][i]+'_'+ls['annee'][i][-2:]+'_TT'+str(ls['TT'][i])+'.can')
            ls_pos.append(float(ls['Z camera'][i]))
            angle = 2.*math.degrees(math.atan2(0.8*math.tan(math.radians(float(ls['angle ouverture'][i]))/2.),1.)) #angle d'ouverture de la camera pov-ray correspondante
